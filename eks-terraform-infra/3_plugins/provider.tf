...
provider "kubernetes" {
  cluster_ca_certificate = try(base64decode(data.terraform_remote_state.eks.outputs.eks.cluster_certificate_authority_data), "")
  token                  = try(data.aws_eks_cluster_auth.this.token, "")
  host                   = try(data.terraform_remote_state.eks.outputs.eks.cluster_endpoint, "")
}
